...
import { ServiceFeeSubCategory, ServiceCategoryEnum } from '../../../../shared/enum/service-category-enum';
...
export class BookHomeCheckoutComponent implements OnInit {
  ...
  async bookUniqueHome() {
    ...
    if (this.isTermAccepted) {
      ...
      this.bookHomeCheckout.svcTypeId = ServiceCategoryEnum.BookUniqueHome;
      try {
        this.loaderService.show();
        const response = await this.bookHomeCheckoutService.bookUniqueHome(this.bookHomeCheckout);
        this.loaderService.hide();
        this.router.navigate(['book/unique-home-thank-you/'+ response.data.Result.genValue]);
      } catch (e) {
        this.loaderService.hide();
      }
    } else {
