...
import { ServiceCategoryEnum, ModalType, ServiceFeeSubCategory } from '../../../../shared/enum/service-category-enum';
...
export class BookUniqueHomeDetailComponent implements OnInit {
  ...
  async getBookUniqueDetail() {
    ...
    try {
      ...
      this.homeDetail.amenities = [];
      this.homeDetail.allAttachments.forEach(attach => {
        if (attach.svcTypeId === ServiceCategoryEnum.HostUniqueHome) {
        ...
        if (attach.svcTypeId === ServiceCategoryEnum.Amenities) {
          if (attach.attachmentType === AttachmentType.Image) {
            const obj = {
              imgSrc: ApiUrl.SRC_URI + attach.attachmentUrl,
              sType: 'img'
            }
            this.homeDetail.amenities.push(obj);
          }
        }
      });
  ...
  openContactToHostModal() {
    this.modalType = ModalType.ContactHost;
    this.contactToHost = true;
  }

  openReviewModal() {
    this.modalType = ModalType.ReviewAndRating;
    this.contactToHost = true;
  }
  ...
  openVideoModal(item) {
    this.videobaseModal = item;
    this.isVideoModal = true;
  }

  vidoeModalPopupEvent(result) {
    this.isVideoModal = result;
  }
